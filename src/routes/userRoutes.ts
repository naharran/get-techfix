...
async function handleAuthentication(email: string, password: string, res: Response) {
    try {
        const authData = await authenticateWithAuth0(email, password);

        // Set a cookie with the token
        res.cookie('authToken', authData.access_token, { httpOnly: true, secure: true, sameSite: 'strict' });
...
router.post('/signup', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const newUser = new User({ email, firstName, lastName,auth0Id:auth0Response.data._id });
        await newUser.save();
...
router.patch('/complete-profile',authenticate, async (req: Request, res: Response) => {
